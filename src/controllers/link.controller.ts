...
import { authenticateSpotify, generateSpotifyAuthUrl, getSpotifyUserInfo } from "../lib/spotify";
    ...
    switch (client_type) {
        case "AI_AGENT":
            res.status(200).json({
                ...
                session: { id: linkSession.id, provider: linkSession.provider, expiresAt: linkSession.expiresAt },
              });
            return;
        case "WEB":
    ...
    const transactionResult = await prisma.$transaction(async (tx) => {
        const linkedAccount = await tx.linkedAccount.create({
            data: {
                ...
                accessTokenExpiresAt: new Date(Date.now() + callbackResponse.expires_in * 1000),
